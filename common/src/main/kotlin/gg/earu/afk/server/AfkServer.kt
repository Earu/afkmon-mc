...
object AfkServer {
    ...
    fun onPlayerLeave(player: ServerPlayer) {
        ...
        broadcast(player.level().server, AfkFlags().toPayload(player.uuid))
    }
    ...
    fun onReport(player: ServerPlayer, payload: AfkPayloads.ReportPayload) {
        ...
        broadcast(player.level().server, next.toPayload(player.uuid, announceSeconds))
    }
    ...
    private fun announce(player: ServerPlayer, afk: Boolean, now: Long): Int {
        ...
        for (listener in player.level().server.playerList.players) {
            if (!canSendTo(listener, AfkPayloads.StatePayload.TYPE)) listener.sendSystemMessage(line)
        }
    ...
    private fun playCue(player: ServerPlayer, afk: Boolean) {
        ...
        val seed = player.level().random.nextLong()
        for (listener in player.level().server.playerList.players) {
            if (listener.level() !== player.level()) continue
    ...
    private fun cue(name: String): Holder<SoundEvent> =
        Holder.direct(SoundEvent.createVariableRangeEvent(Identifier.fromNamespaceAndPath(Afk.MOD_ID, name)))
